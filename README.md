...
| Plugin | Description |
|--------|-------------|
| **codex-review** | Consensus code review through deliberation between Claude and Codex. Both models independently review code, then debate findings until they agree. Requires [codex](https://github.com/openai/codex) installed and on PATH. |
...
Source files:
- [`test-fixtures/eval-1/auth.js`](plugins/codex-review/test-fixtures/eval-1/auth.js) — Express authentication with SQL injection, plaintext passwords, hardcoded credentials, and user data leaks
- [`test-fixtures/eval-2/queue.py`](plugins/codex-review/test-fixtures/eval-2/queue.py) — Job queue with a `threading.Lock` that is created but never acquired
- [`test-fixtures/eval-3/cache.rs`](plugins/codex-review/test-fixtures/eval-3/cache.rs) — Thread-safe cache with TOCTOU races, unnecessary `unsafe`, broken eviction, and cleanup thread issues
